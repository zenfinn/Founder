...
import { withDisplayMemberCount } from "@/lib/community-display";
...
export function isProLoungeCommunity(group) {
  const slug = String(group?.slug ?? "").toLowerCase();
  const name = String(group?.name ?? "").toLowerCase();
  return slug === "founder-pro" || name === "founder pro lounge";
}
...
export async function listCommunitiesForUser(supabase, userId) {
  ...
  const loungeGroup = groups?.find((group) => isGlobalLounge(group));
  if (loungeGroup) membershipIds.delete(loungeGroup.id);
  ...
  const communities = (groups ?? [])
    .filter((group) => !isProLoungeCommunity(group) && !isGlobalLounge(group))
    .reduce((unique, group) => {
      const key = group.slug ?? group.category ?? group.id;
      if (unique.some((item) => (item.slug ?? item.category ?? item.id) === key)) {
        return unique;
      }
      unique.push(group);
      return unique;
