...
function mapResourcePost(row) {
  const meta = row.metadata ?? {};
  const upvotes = meta.upvotes ?? 0;
  const downvotes = meta.downvotes ?? 0;
...
export async function getResourceRankings(supabase, groupId) {
  const { data, error } = await supabase
    .from("posts")
...
export async function createResource(supabase, { groupId, userId, title, url, type }) {
  const { data, error } = await supabase
    .from("posts")
    .insert({
      group_id: groupId,
