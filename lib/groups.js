...
import { fetchAllLegacyTools, fetchLegacyToolsForGroup, mergeCommunityTools } from "@/lib/community-tools";
...
function mapResourcePost(row, { listIndex = null, viewerId = null } = {}) {
...
export async function getResourceRankings(supabase, groupId, { viewerId = null } = {}) {
  ...
  const postTools = (data ?? [])
    ...
    .map((row, index) => mapResourcePost(row, { listIndex: index, viewerId }));

  const legacyTools = group ? await fetchLegacyToolsForGroup(supabase, group) : [];
...
export async function getAllApprovedResources(supabase) {
  ...
  const postTools = (data ?? [])
    .filter(isApprovedResourceRow)
    .map((row, index) => mapResourcePost(row, { listIndex: index }));
