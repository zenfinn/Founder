...
const MEETUP_INTENT_PATTERN =
  /(?:meetup|meet-?up).*(?:organisier|planen|einreichen|machen|erstellen)|(?:organisier|planen|einreichen|erstellen).*(?:meetup|meet-?up|event|treffen|workshop)|event\s+(organisier|planen|einreichen)|treffen\s+organisier|workshop\s+(organisier|planen)|networking\s+(event|treffen)\s+organisier|veranstaltung\s+(organisier|planen|einreichen)|ich\s+will\s+(?:ein\s+)?(?:meetup|event|treffen|workshop)/i;

const GREETING_PATTERN =
  /^(hallo|hey|hi|moin|servus|yo|hello|guten\s+(tag|morgen|abend))(?:\s+(jarvis|founder))?\s*[!.?]*$/i;
...
export function hasExplicitMeetupIntent(messages = []) {
  return messages
    .filter((message) => message.role === "user")
    .some((message) => userWantsMeetupFlow(message.text));
}

export function userRejectsMeetupFlow(text = "") {
  const value = String(text ?? "").toLowerCase();
  return (
    /was findet|wovon redest|kein meetup|nicht meetup|kein event|was meinst|versteh.*nicht|warum fragst|anderes thema|was soll das|nicht darum|geht nicht um/i.test(
      value
    ) || userCancelsFlow(text)
  );
}
...
export function isMeetupFlowActive(meetup = {}, messages = []) {
  ...
  if (userRejectsMeetupFlow(lastUser)) return false;
  return hasExplicitMeetupIntent(messages);
}
...
export function extractMeetupFromText(text = "", { expectingField = null, meetup = {} } = {}) {
  const value = String(text ?? "").trim();
  if (!value || !expectingField) return {};

  const patch = {};

  if (expectingField === "title" && value.length >= 3 && !isGreetingOrSmalltalk(value)) {
    patch.title = value.slice(0, 160);
  }

  if (expectingField === "starts_at") {
  ...
  if (expectingField === "host_info" && value.length >= 8) {
