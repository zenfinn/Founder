...
/** Demo / TikTok accounts — onboarding never completes, fresh start each visit. */
export const FORCE_ONBOARDING_EMAILS = ["finn_zender@icloud.com"];

export function normalizeFounderEmail(email = "") {
  return String(email ?? "").trim().toLowerCase();
}

export function isForceOnboardingEmail(email = "") {
  return FORCE_ONBOARDING_EMAILS.includes(normalizeFounderEmail(email));
}
...
function readOnboardingStatus(userId) {
  ...
  try {
    return window.localStorage.getItem(getOnboardingStorageKey(userId));
  } catch {
    return null;
  }
}

export function shouldShowFounderOnboarding(userId, email = "") {
  if (isForceOnboardingEmail(email)) return true;
  const status = readOnboardingStatus(userId);
  return status !== "done" && status !== "skipped";
}

export function readOnboardingComplete(userId, email = "") {
  if (isForceOnboardingEmail(email)) return false;
  const status = readOnboardingStatus(userId);
  return status === "done" || status === "skipped";
}

export function writeOnboardingComplete(userId, email = "") {
  if (isForceOnboardingEmail(email)) return;
  if (typeof window === "undefined" || !userId) return;
  window.localStorage.setItem(getOnboardingStorageKey(userId), "done");
}

export function writeOnboardingSkipped(userId, email = "") {
  if (isForceOnboardingEmail(email)) return;
  if (typeof window === "undefined" || !userId) return;
  window.localStorage.setItem(getOnboardingStorageKey(userId), "skipped");
}
