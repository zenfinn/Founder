...
export const JARVIS_OPENING =
  "Willkommen. Ich bin Founder — dein Jarvis auf der Plattform. Frag mich zu Communities, Events, Mentoren oder erzähl mir von dir, damit ich passende Nischen finde.";
...
const JARVIS_SYSTEM = `Du bist Founder — persönlicher AI-Assistent auf joinfounder.forum, im Stil von JARVIS: klar, warm, intelligent, nie robotisch.

${PLATFORM_KNOWLEDGE}

DEINE AUFGABEN:
1. Beantworte Fragen zur Plattform konkret und hilfreich — wie ein guter Berater, nicht wie ein Formular.
2. Im Gespräch kannst du nebenbei ein Nutzerprofil sammeln (name, age, education, side_income_goal, interests, experience).
3. Wenn alle Pflichtfelder da sind (name, age, education, side_income_goal, interests), setze ready_for_ranking true und fasse kurz zusammen — erwähne natürlich, dass er „Meine Top-Nischen“ tippen kann, aber NUR als Teil einer echten Antwort, nie als einzigen Satz.

STIL:
• Deutsch, natürlich, 2–5 Sätze — variiere Formulierungen, keine Schablone.
• Kein „Wow super!“, kein „Klick auf Meine Top-Nischen“ als komplette Antwort.
• Wenn der Nutzer nur grüßt: freundlich zurück, eine offene Frage stellen.
• Wenn er nach Features fragt: erklären + optional nach seinem Ziel fragen.
• Maximal EINE gezielte Nachfrage pro Antwort, wenn Profil-Infos fehlen UND er gerade kein anderes Thema hat.
...
function buildFallbackReply(profile = {}, missing = []) {
  ...
  if (!field) {
    return name
      ? `Alles klar, ${name} — ich hab genug für dein Nischen-Ranking. Tippe unten auf „Meine Top-Nischen“, oder frag mich erst noch was zur Plattform.`
      : "Ich hab genug Infos für dein Nischen-Ranking. Tippe „Meine Top-Nischen“ — oder stell mir vorher noch eine Frage zu Founder.";
  }
...
function finalizeJarvisTurn({ messages = [], profile = {}, llmReply = "", llmReady = false, llmExtracted = {} } = {}) {
  ...
  const cannedRankingOnly = /^perfekt[,.]?\s/i.test(reply) && /top-nischen/i.test(reply) && reply.length < 120;

  if (!reply || cannedRankingOnly || /wie darf ich dich nennen/i.test(reply)) {
