...
function collectInterestsFromMessages(messages = []) {
  ...
  for (let index = 0; index < messages.length; index += 1) {
    ...
    const expectingField = askedInterests ? "interests" : null;

    collected.push(
      ...extractInterestsFromText(message.text),
      ...parseInterestTopicsFromText(message.text, { expectingField })
    );

    if (expectingField === "interests" && message.text.trim().length >= 4 && !isIncomeAnswer(message.text)) {
      const parts = message.text
        .split(/,|\bund\b|\boder\b/i)
        .map((part) => part.trim())
        .filter((part) => isUserStatedInterest(part));
      collected.push(...parts);
      if (!parts.length && isUserStatedInterest(message.text)) collected.push(message.text.trim().slice(0, 80));
    }
  }
...
function collectExperienceFromMessages(messages = []) {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    ...
    const priorFounder = [...messages.slice(0, index)]
      .reverse()
      .find((entry) => entry.role === "founder");
    const askedExperience = priorFounder && /ausprobiert|erfahrung|schon gemacht/i.test(priorFounder.text);
...
function isCasualGreeting(text = "") {
  ...
  return /^(hallo|hey|hi|moin|servus|yo|hello)(\s+[a-zäöüß][a-zäöüß0-9_-]{1,})?[!.?\s]*$/i.test(value);
}
...
function personalizeReplyAddress(reply = "", name = "", { aboutSelf = true } = {}) {
  ...
  if (aboutSelf) {
    if (!nameAtStart.test(text) && /\b(du|dich|dir|dein|deine|deinen|bist du|hast du)\b/i.test(text)) {
...
function extractNameFromText(text = "") {
  ...
  const patterns = [
    ...
    /(?:darfst|darf|sollst|kannst|will|würdest|dürft)\s+(?:du\s+)?mich\s+(?:gerne\s+|bitte\s+)?([A-Za-zÄÖÜäöüß][A-Za-zÄÖÜäöüß0-9_-]{1,})(?:\s+nennen)?/i,
...
function extractNameAnswer(text = "", { expectingField = false } = {}) {
  ...
  if (isCasualGreeting(value)) {
    const greetingName = value.match(/^(?:hallo|hey|hi|moin|servus|yo|hello)\s+([a-zäöüß][a-zäöüß0-9_\s-]{1,})$/i)?.[1]?.trim();
    if (greetingName && isValidProfileName(greetingName) && !isAssistantAlias(greetingName)) return greetingName;
    return null;
  }
...
function extractFromUserText(text = "", { expectingField = null, profile = {} } = {}) {
  ...
  if ((expectingField === "name" || !profile.name) && !isCasualGreeting(value)) {
  ...
  if (!field) {
    ...
    return name
      ? aboutSelf
        ...
        : `Gute Basis. Wenn ${name} bereit ist, tippe unten auf „Meine Top-Nischen“.`
...
function buildInterestAckReply(profile = {}, interests = [], { aboutSelf = true } = {}) {
  ...
  if (aboutSelf) {
    return name
      ? `Verstanden, ${name} — ${topics} klingt spannend. Was hast du in dem Bereich schon ausprobiert?`
...
function finalizeJarvisTurn({
  ...
  if (mode === "onboarding" && isGreetingToAssistant(lastUserText)) {
  ...
  if (mode === "assistant") {
    ...
    (expectingField === "name" || missing.includes("age")) &&
    fromConversation.name &&
    /nennen|heiß|heiss|name/i.test(lastUserText) &&
