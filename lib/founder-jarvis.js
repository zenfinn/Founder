...
const MIN_USER_TURNS_FOR_RANKING = 6;
...
function buildFallbackReply(profile = {}, missing = [], messages = []) {
  ...
  if (!field) {
    ...
    return name
      ? `Gute Basis, ${name}. Wenn du bereit bist, tippe unten auf „Meine Top-Nischen“.`
...
function buildSmartNicheSuggestionReply(userText = "", profile = {}, messages = []) {
  ...
  if (missing.includes("experience")) {
    followUp = " Was hast du in dem Bereich schon ausprobiert?";
  } else if (missing.includes("side_income_goal")) {
    followUp = " Was ist dein Nebenverdienst-Ziel pro Monat?";
  }

  return `Gute Richtung — zu dem was du sagst, passen auf Founder besonders:\n\n${lines
    .slice(0, 3)
    .map((line) => `• ${line}`)
    .join("\n")}\n\nSchau unter **Community** in der App — oder stell mir noch eine Frage.${followUp}`;
}
...
function finalizeJarvisTurn({ messages = [], profile = {}, llmReply = "", llmReady = false, llmExtracted = {} } = {}) {
  ...
  if (smartNiche && (!reply || cannedRankingOnly || isCannedInterestPrompt(reply))) {
    reply = smartNiche;
  } else if (!reply || cannedRankingOnly) {
    reply = buildFallbackReply(fromConversation, missing, messages);
  }

  if (/top-nischen/i.test(reply) && !ready) {
...
function chatWithJarvisFallback({ messages = [], profile = {} }) {
  ...
  return finalizeJarvisTurn({
    messages,
    profile: fromConversation,
    llmReply: buildFallbackReply(fromConversation, missing, messages),
    llmReady: false,
  });
}
