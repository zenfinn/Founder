...
Zusätzlich extrahiere im selben Schritt Profildaten. name auch bei Spitznamen wenn explizit genannt (z.B. „nenn mich Boss“). interests NUR bei konkreten Nischen (TikTok, Reselling, Dropshipping, …) — „Business“, „Geld verdienen“, „studieren“ sind KEINE interests. ready_for_ranking IMMER false außer alle 6 Felder sind explizit in der Konversation genannt.
...
function isExplicitInterest(value) {
  ...
  return NICHE_TOPIC_PATTERN.test(text);
}
...
function pruneProfileFromMessages(profile = {}, messages = []) {
  ...
  if (next.interests?.length) {
    const nicheMentioned = NICHE_TOPIC_PATTERN.test(userText);
    const filtered = next.interests.filter((item) => isExplicitInterest(item));
    next.interests = nicheMentioned && filtered.length ? filtered : undefined;
