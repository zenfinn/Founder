...
function extractNameAnswer(text = "", { expectingField = false } = {}) {
  ...
  if (isCasualGreeting(value)) {
    const greetingName = value.match(/^(?:hallo|hey|hi|moin|servus|yo|hello)\s+([a-zäöüß][a-zäöüß0-9_-]{1,})$/i)?.[1];
    if (greetingName && isValidProfileName(greetingName)) return greetingName;
    return null;
  }
...
function finalizeJarvisTurn({
  ...
  } else if (missing[0] === "name" && isCasualGreeting(lastUserText) && !fromConversation.name) {
