...
function pruneProfileFromMessages(profile = {}, messages = []) {
  ...
  if (next.name && nameFromUser && next.name.toLowerCase() === nameFromUser.toLowerCase()) {
    // keep — clearly extracted from conversation
  } else if (next.name && !nameFromUser) {
    delete next.name;
  }
...
function hasExplicitNamePhrase(text = "") {
  return Boolean(extractNameFromText(text));
}
