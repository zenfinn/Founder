...
Zusätzlich extrahiere im selben Schritt Profildaten. interests nur bei explizit genannten Nischen/Themen — nie aus Studium ableiten. ready_for_ranking nur wenn name, age, education, side_income_goal, interests und experience alle klar vom Nutzer stammen.
...
function extractNameFromText(text = "") {
  ...
  const explicit = value.match(/(?:ich bin|ich heiße|ich heisse|name ist|nenn mich)\s+([A-Za-zÄÖÜäöüß][A-Za-zÄÖÜäöüß-]{1,})/i);
  const candidate = explicit?.[1]?.trim();
  return candidate && isValidProfileName(candidate) ? candidate : null;
}
...
function extractFromUserText(text = "", { expectingField = null, profile = {} } = {}) {
  ...
  if (expectingField === "name") {
    const fromPhrase = extractNameFromText(value);
    if (fromPhrase) patch.name = fromPhrase;
    else if (isValidProfileName(value.split(/\s+/)[0])) patch.name = value.split(/\s+/)[0];
