...
import {
  buildMeetupConfirmReply,
  buildMeetupFallbackReply,
  extractAllMeetupFromConversation,
  getMissingMeetupFields,
  isMeetupFlowActive,
  isMeetupReadyForSubmit,
  mergeExtractedMeetup,
  userCancelsFlow,
  userConfirmsAction,
...
const JARVIS_MEETUP_BLOCK = `
MEETUP / EVENT EINREICHEN (nur bei expliziter Anfrage):
• meetup_flow_active NUR true wenn der Nutzer KLAR sagt, dass er ein Meetup/Event/Treffen/Workshop organisieren oder einreichen will.
• NICHT bei Begrüßungen (hallo, hey), Smalltalk, allgemeinen Fragen oder wenn nur „Jarvis“/„Founder“ genannt wird.
• Beispiele für explizite Anfrage: „Ich will ein Meetup organisieren“, „Kann ich ein Event einreichen?“, „Hilf mir ein Treffen zu planen“.
• Erst nach expliziter Anfrage Pflichtfelder sammeln: title, starts_at, location_text, description.
• Optional: category (Standard „Meetup“), host_info.
• Wenn alle Pflichtfelder da: zusammenfassen und „Soll ich das so einreichen?“ — ready_for_submit=true.
• Wenn der Nutzer verwirrt ist oder das Thema wechselt: meetup_flow_active=false, extracted_meetup leeren, normal antworten.`;
...
const JARVIS_CHAT_JSON_BASE = `Antworte NUR als JSON:
{
  ...
  "extracted_meetup": {
    ...
    "starts_at": "ISO datetime string or null",
    "location_text": "string or null",
    "description": "string or null",
    "host_info": "string or null",
    "category": "string or null"
  },
  "meetup_flow_active": boolean,
  "ready_for_submit": boolean,
  "meetup_awaiting_confirm": boolean,
...
const GENERIC_INTEREST_PHRASES =
  /^(business|unternehmen|unternehmertum|geld verdienen|side hustle|kleines business|was aufbauen|online business|selbstständig|gründen)$/i;

const NICHE_TOPIC_PATTERN =
  /tiktok|e-?commerce|resell|dropship|immobilien|real estate|trading|aktien|aktienmarkt|börse|borsen|forex|daytrade|produktverkauf|produkt(e)?\s*verkauf|waren|verkauf|crypto|memecoin|ki\b|ai\b|creator|youtube|web.?design|amazon|fba|podcast|handwerk|dienstleist/i;

/** Maps natural-language topics to platform interest labels + community slugs. */
const INTEREST_TOPIC_ALIASES = [
  { pattern: /aktienmarkt|aktien|börse|borsen|forex|daytrade|trading|chart/i, label: "Trading", slugs: ["trading"] },
  { pattern: /produktverkauf|produkte?\s*verkaufen|warenverkauf|weiterverkauf|resell|flipping/i, label: "Reselling", slugs: ["reselling", "e-commerce"] },
  { pattern: /online\s*shop|e-?commerce|eigenen\s*shop|dtc|produkt\s*verkauf/i, label: "E-Commerce", slugs: ["e-commerce", "dropshipping"] },
...
function isExplicitInterest(value) {
  ...
  return INTEREST_TOPIC_ALIASES.some((alias) => alias.label.toLowerCase() === text.toLowerCase());
}
...
function pruneProfileFromMessages(profile = {}, messages = []) {
  ...
  if (next.interests?.length) {
    ...
    const merged = [...(next.interests ?? []), ...detected];
    const filtered = merged.filter((item) => isExplicitInterest(item));
    next.interests = userMentionsNicheTopics(userText) && filtered.length ? [...new Set(filtered)] : undefined;
    if (!next.interests?.length) delete next.interests;
  } else if (userMentionsNicheTopics(userText)) {
    const detected = extractInterestsFromText(userText);
    if (detected.length) next.interests = detected;
  }
...
function buildProfileContext(profile = {}, missing = [], meetup = {}, meetupMissing = []) {
  const known = Object.entries(profile)
    .filter(([, value]) => value != null && value !== "" && !(Array.isArray(value) && !value.length))
    .map(([key, value]) => `${key}: ${Array.isArray(value) ? value.join(", ") : value}`)
    .join("; ");

  const meetupKnown = Object.entries(meetup)
...
Fehlende Profilfelder: ${missing.length ? missing.join(", ") : "keine — Ranking möglich"}.
Meetup-Entwurf: ${meetupKnown || "noch leer"}.
Fehlende Meetup-Felder: ${meetupMissing.length ? meetupMissing.join(", ") : "keine — Einreichung möglich"}.`;
}
...
function extractFromUserText(text = "", { expectingField = null, profile = {} } = {}) {
  ...
  } else if (expectingField === "interests" || userMentionsNicheTopics(value)) {
    const parts = value
      .split(/,| und /i)
      .map((part) => part.trim())
...
function buildSmartNicheSuggestionReply(userText = "", profile = {}, messages = []) {
  ...
  const lines = [];
  if (/aktienmarkt|aktien|börse|borsen|trading|forex/i.test(userText)) {
    lines.push(
  ...
  if (/produktverkauf|produkt|verkauf|resell|waren/i.test(userText)) {
    lines.push(
...
function finalizeJarvisTurn({
  ...
  llmMeetup = {},
  llmMeetupFlow = false,
  llmReadySubmit = false,
  llmAwaitingConfirm = false,
  llmMeetupConfirmed = false,
  mode = "onboarding",
} = {}) {
  const merged = sanitizeProfile(mergeExtractedProfile(profile, llmExtracted));
  const fromConversation = sanitizeProfile(pruneProfileFromMessages(extractAllFromConversation(messages, merged), messages));
  const missing = getMissingProfileFields(fromConversation);
  const ready = isProfileReadyForRanking(fromConversation, messages);

  let nextMeetup = mergeExtractedMeetup(meetup, llmMeetup);
  const meetupFlow = isMeetupFlowActive(nextMeetup, messages);
  nextMeetup = meetupFlow ? extractAllMeetupFromConversation(messages, nextMeetup) : {};
  const activeMeetup = meetupFlow ? nextMeetup : {};
  const missingMeetup = meetupFlow ? getMissingMeetupFields(activeMeetup) : [];
  const meetupReady = meetupFlow && isMeetupReadyForSubmit(activeMeetup);

  let reply = String(llmReply ?? "").trim();
  const lastUserText = messages.filter((message) => message.role === "user").at(-1)?.text ?? "";
  const userConfirmedMeetup = userConfirmsAction(lastUserText);
  ...
  if (meetupFlow && missingMeetup.length > 0) {
    if (!reply || reply.length < 20) {
      reply = buildMeetupFallbackReply(activeMeetup, missingMeetup, fromConversation.name);
    }
  } else if (userRejectsMeetupFlow(lastUserText) && /wann findet|meetup|datum und uhrzeit/i.test(reply)) {
    reply =
      buildAssistantDefaultReply(fromConversation) ||
      "Alles klar — womit kann ich dir helfen? Wenn du ein Meetup planen willst, sag einfach Bescheid.";
  } else if (meetupFlow && meetupReady) {
    if (!userConfirmedMeetup && (!reply || !/einreichen/i.test(reply))) {
      reply = buildMeetupConfirmReply(activeMeetup, fromConversation.name);
    } else if (userConfirmedMeetup || llmMeetupConfirmed) {
      reply =
        reply ||
        (fromConversation.name
          ? `Alles klar, ${fromConversation.name} — ich reiche dein Meetup jetzt ein.`
          : "Alles klar — ich reiche dein Meetup jetzt ein.");
    }
  ...
  if (!meetupFlow && mode === "onboarding" && /top-nischen/i.test(reply) && !ready) {
  ...
  const meetupAwaitingConfirm =
    meetupReady && (llmAwaitingConfirm || (!userConfirmedMeetup && !llmMeetupConfirmed));
  ...
  return {
    ...
    meetup: activeMeetup,
    meetupFlow,
    readyForSubmit: meetupReady,
    meetupAwaitingConfirm,
    meetupConfirmed: meetupReady && (userConfirmedMeetup || llmMeetupConfirmed),
    missingMeetup,
...
function chatWithJarvisFallback({ messages = [], profile = {}, meetup = {}, mode = "onboarding" } = {}) {
  const fromConversation = pruneProfileFromMessages(extractAllFromConversation(messages, profile), messages);
  const missing = getMissingProfileFields(fromConversation);
  const lastUserText = messages.filter((message) => message.role === "user").at(-1)?.text ?? "";

  const meetupFlow = isMeetupFlowActive(meetup, messages);
  const meetupDraft = meetupFlow ? extractAllMeetupFromConversation(messages, meetup) : {};
  const missingMeetup = meetupFlow ? getMissingMeetupFields(meetupDraft) : [];

  let llmReply;
  if (meetupFlow && missingMeetup.length) {
    llmReply = buildMeetupFallbackReply(meetupDraft, missingMeetup, fromConversation.name);
  } else if (meetupFlow && isMeetupReadyForSubmit(meetupDraft)) {
    llmReply = userConfirmsAction(lastUserText)
      ? fromConversation.name
        ? `Alles klar, ${fromConversation.name} — ich reiche dein Meetup jetzt ein.`
        : "Alles klar — ich reiche dein Meetup jetzt ein."
      : buildMeetupConfirmReply(meetupDraft, fromConversation.name);
  } else {
    llmReply =
...
export async function chatWithJarvis({ messages = [], profile = {}, meetup = {}, mode = "onboarding" } = {}) {
  if (!isOpenAiVoiceConfigured()) {
    return chatWithJarvisFallback({ messages, profile, meetup, mode });
  }
  ...
  const meetupDraft = meetupFlow ? extractAllMeetupFromConversation(messages, meetup) : {};
  const missingMeetup = meetupFlow ? getMissingMeetupFields(meetupDraft) : [];
  const profileContext = buildProfileContext(profile, missing, meetupDraft, missingMeetup);
  ...
  try {
    ...
    return finalizeJarvisTurn({
      ...
      llmMeetup: parsed.extracted_meetup ?? {},
      llmMeetupFlow: Boolean(parsed.meetup_flow_active),
      llmReadySubmit: Boolean(parsed.ready_for_submit),
      llmAwaitingConfirm: Boolean(parsed.meetup_awaiting_confirm),
      llmMeetupConfirmed: Boolean(parsed.meetup_confirmed),
      mode,
    });
  } catch (error) {
