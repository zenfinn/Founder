...
function extractExperienceFromText(text = "") {
  ...
  const experiential =
    /noch gar nichts|gar nichts|noch nichts|nichts gemacht|nichts in|habe|hatte|schon|ausprobiert|versucht|gemacht|trade republic|aktien|börse|noch nicht|erste mal|anfäng|beginn|erfahrung|nutze|benutze|mache|mache seit|gerade erst/i.test(
      value
    );

  if (experiential) return value.slice(0, 200);
  return null;
}
...
function isIncomeAnswer(text = "") {
  ...
  return (
    /\d/.test(value) &&
    /\b(pro monat|monatlich|monat|nebenbei|verdienen|verdienst|€|euro|eur|zu verdienen)\b/i.test(value) &&
...
export function isProfileReadyForRanking(profile = {}, messages = []) {
  ...
  if (getMissingProfileFields(pruned).length > 0) return false;
  if (messages.length > 0) {
    if (!collectInterestsFromMessages(messages).length) return false;
    if (!collectExperienceFromMessages(messages)) return false;
  }
  return true;
}
...
function personalizeReplyAddress(reply = "", name = "", { aboutSelf = true } = {}) {
  ...
  if (!displayName || !isValidProfileName(displayName) || !String(reply ?? "").trim()) return reply;
...
function extractFromUserText(text = "", { expectingField = null, profile = {} } = {}) {
  ...
  if (expectingField === "name" && !isCasualGreeting(value)) {
    const nameAnswer = extractNameAnswer(value, { expectingField: true });
  ...
  if (expectingField === "side_income_goal" || !profile.side_income_goal) {
    ...
    } else if ((expectingField === "side_income_goal" || isIncomeAnswer(value)) && /\d/.test(value)) {
...
function buildFallbackReply(profile = {}, missing = [], messages = []) {
  ...
  if (field === "experience" && messages.length && !collectInterestsFromMessages(messages).length) {
    return buildFallbackReply(profile, ["interests", ...missing.filter((entry) => entry !== "interests")], messages);
  }
  ...
  if (!field) {
    const pruned = messages.length ? pruneProfileFromMessages(profile, messages) : sanitizeProfile(profile);
    const realMissing = getMissingProfileFields(pruned);
    if (realMissing.length) return buildFallbackReply(profile, realMissing, messages);

    const actuallyReady = isProfileReadyForRanking(profile, messages);
    if (!actuallyReady) {
...
function summarizeExperienceForReply(experience = "") {
  ...
  if (
    /noch\s+(?:gar\s+)?nichts|noch\s+nicht|keine\s+erfahrung|habe\s+nichts|noch\s+nie|gerade\s+erst|nichts\s+in\s+(?:dem|diesem)|nichts\s+gemacht/i.test(
      text
    )
  ) {
  ...
  if (
    /\b(?:schon|bereits)\b|ausprobiert|nutze\s+seit|mache\s+seit/i.test(text) &&
    !/nichts|noch\s+nicht|noch\s+nie|gar\s+nichts/i.test(text)
  ) {
...
function buildExperienceAckReply(profile = {}, { aboutSelf = true, ready = false, missing = [], messages = [] } = {}) {
  const name = profile.name;
  ...
  if (missing.includes("interests") || (messages.length && !collectInterestsFromMessages(messages).length)) {
    return getFallbackPrompt("interests", name, { aboutSelf });
  }

  return buildFallbackReply(profile, missing, messages);
}
...
function finalizeJarvisTurn({
  ...
  if (mode === "assistant") {
    ...
    !getMissingProfileFields(fromConversation).includes("side_income_goal")
  ) {
    if (!collectInterestsFromMessages(messages).length) {
      delete fromConversation.interests;
      missing = getMissingProfileFields(fromConversation);
    }
    if (missing.includes("interests") || !collectInterestsFromMessages(messages).length) {
    ...
    collectInterestsFromMessages(messages).length
  ) {
    ...
    missing = getMissingProfileFields(fromConversation);
    reply = buildExperienceAckReply(fromConversation, { aboutSelf, ready, missing, messages });
  } else if (
  ...
  if (mode === "onboarding" && /ich bereite jetzt/i.test(reply) && !ready) {
    reply = buildFallbackReply(fromConversation, missing, messages);
  }
