...
function finalizeJarvisTurn({
  ...
  const explicitNameFromUser = hasExplicitNamePhrase(lastUserText) ? collectNameFromMessages(messages) : null;
  ...
  if (mode === "assistant") {
    ...
    hasExplicitNamePhrase(lastUserText) &&
    ...
    if (!reply && knowledgeReply) reply = knowledgeReply;
      ...
      (/wie darf ich dich nennen/i.test(reply) && hasExplicitNamePhrase(lastUserText))
    ) {
  ...
  if (/wie darf ich dich nennen/i.test(reply) && (fromConversation.name || hasExplicitNamePhrase(lastUserText))) {
