...
MEETUP / EVENT EINREICHEN (über Jarvis):
• Wenn der Nutzer ein Meetup, Event, Treffen oder Workshop organisieren will: meetup_flow_active=true.
• Pflichtfelder: title, starts_at (ISO 8601, Europe/Berlin), location_text, description.
• Optional: category (Standard „Meetup“), host_info (wer organisiert).
• Fehlende Felder nacheinander erfragen — freundlich und konkret, nicht wie ein Formular.
• Wenn alle Pflichtfelder da: kurz zusammenfassen und fragen „Soll ich das so einreichen?“ — ready_for_submit=true, meetup_awaiting_confirm=true.
• Wenn der Nutzer bestätigt (ja, einreichen, passt): meetup_confirmed=true — kurz bestätigen, das System reicht ein.
• Wenn der Nutzer ablehnt oder ändern will: meetup_confirmed=false, nach Korrekturen fragen.
• NIEMALS behaupten, dass eingereicht wurde, bevor der Nutzer bestätigt hat.
• Während Meetup-Flow: Profilfragen nicht unterbrechen, außer der Nutzer wechselt das Thema.`;
...
function finalizeJarvisTurn({
  ...
  nextMeetup = extractAllMeetupFromConversation(messages, nextMeetup);
  const meetupFlow = (llmMeetupFlow || isMeetupFlowActive(nextMeetup, messages)) && !userCancelsFlow(
...
function chatWithJarvisFallback({ messages = [], profile = {}, meetup = {}, mode = "onboarding" } = {}) {
  ...
  const meetupDraft = extractAllMeetupFromConversation(messages, meetup);
  const meetupFlow = isMeetupFlowActive(meetupDraft, messages);
  const missingMeetup = getMissingMeetupFields(meetupDraft);
