...
export function writeOnboardingProgress(userId, progress) {
  ...
  window.dispatchEvent(new CustomEvent("founder-onboarding-update"));
}
...
export function isCommunityStepComplete({ communitiesCount, profile }) {
  if (hasFullAccess(profile)) return true;
  return communitiesCount > 0;
}

export function getAutoCompletedSteps({ profile, verificationStatus, currentRank, communitiesCount }) {
  return {
    profile: isProfileStepComplete(profile),
    verify: isVerifyStepComplete({ verificationStatus, currentRank, profile }),
    community: isCommunityStepComplete({ communitiesCount, profile }),
  };
}
