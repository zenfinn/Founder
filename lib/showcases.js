...
export function getShowcaseImageUrls(row = {}) {
  const raw = row.image_urls;
  if (Array.isArray(raw) && raw.length > 0) {
    return raw.map((url) => String(url).trim()).filter(Boolean).slice(0, MAX_SHOWCASE_IMAGES);
  }
  if (typeof raw === "string") {
    try {
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed) && parsed.length > 0) {
        return parsed.map((url) => String(url).trim()).filter(Boolean).slice(0, MAX_SHOWCASE_IMAGES);
      }
    } catch {
      /* ignore */
    }
...
export function parseShowcaseImageUrls(body = {}) {
  if (Array.isArray(body.image_urls)) {
    const urls = body.image_urls.map((url) => String(url ?? "").trim()).filter(Boolean);
    if (urls.length) return urls.slice(0, MAX_SHOWCASE_IMAGES);
  }
  const single = String(body.image_url ?? "").trim();
  return single ? [single] : [];
}

export function buildShowcaseImageFields(imageUrls) {
  const urls = (imageUrls ?? []).slice(0, MAX_SHOWCASE_IMAGES);
  return {
    image_url: urls[0] ?? "",
    image_urls: urls,
  };
