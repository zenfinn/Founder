...
function isMobileApple() {
  ...
  return /iPhone|iPad|iPod/i.test(ua) || (/AppleWebKit/i.test(ua) && /Safari/i.test(ua) && !/Chrome/i.test(ua));
}
...
export async function playFounderMp3Blob(blob) {
  ...
  // iOS/Safari: HTML Audio ist zuverlässiger als Web Audio decode
  if (isMobileApple()) {
    try {
      return await tryHtmlAudio();
    } catch {
