...
export function isPlaceholderLegacyUrl(url) {
...
export function parseLegacyResourceId(resourceId) {
  const match = String(resourceId ?? "").match(/^(db|seed)-(.+)$/);
  if (!match) return null;

  return { source: match[1], recordId: match[2] };
}

export async function deleteLegacyGroupResource(supabase, resourceId) {
  const parsed = parseLegacyResourceId(resourceId);
  if (!parsed || parsed.source !== "db") {
    throw new Error("Dieses kuratierte Tool kann nicht gelöscht werden.");
  }

  const { error } = await supabase.from("group_resources").delete().eq("id", parsed.recordId);
  if (error) throw error;
}

function isDisplayableLegacyTool(tool) {
  return !isPlaceholderLegacyUrl(tool.url);
}
...
function dedupeTools(tools) {
  ...
  for (const tool of tools) {
    ...
    if (!existing) {
      byTitle.set(titleKey, tool);
      continue;
    }

    if (existing.legacy && !tool.legacy) {
      byTitle.set(titleKey, tool);
    }
  }
...
export function mergeCommunityTools(postTools, legacyTools) {
  const visibleLegacy = legacyTools.filter(isDisplayableLegacyTool);
  const deduped = dedupeTools([...postTools, ...visibleLegacy]);
