...
const MOBILE_APPLE = () => typeof window !== "undefined" && (isSafari() || isIos());
...
export async function speakFounderText(text, options = {}) {
  ...
  unlockFounderAudioPlayback();
  const apiVoice = await fetchFounderVoiceStatus();
  const mobileApple = MOBILE_APPLE();
  ...
  if (apiVoice) {
    try {
      ...
      try {
        await speakFounderTextInBrowser(text, { rate: 0.82, pitch: 0.62, volume: 1, ...options });
  ...
  if (mobileApple) {
    try {
      ...
      console.warn("Founder voice: browser TTS failed on mobile");
      return { engine: "none" };
    }
  }
